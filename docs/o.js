...
async function gubs() {
	OT = new ethers.Contract(OTOKEN, ["function balanceOf(address) public view returns(uint)"], signer);
	_obal = await OT.balanceOf(window.ethereum.selectedAddress);
	$("bal_o").innerHTML = fornum(Number(_obal),18);
	return;
...
async function oRetroToElRetro() {
	...
	notice(`
		...
		<img style='height:20px;position:relative;top:4px' src="${BASELOGO}"> oToken Exercised: <b>${fornum(_oamt,18)}</b><br>
		<img style='height:20px;position:relative;top:4px' src="${WRAPLOGO}"> ${WRAPNAME} Expected: <b>${fornum(_wamt,18)}</b><br>
